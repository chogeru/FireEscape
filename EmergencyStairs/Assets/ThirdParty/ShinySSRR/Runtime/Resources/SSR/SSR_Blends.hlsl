...
#define SSR_BLENDS
    ...
    half4 FragDebugTransparentNormals (VaryingsSSR i) : SV_Target {
        UNITY_SETUP_INSTANCE_ID(i);
        UNITY_SETUP_STEREO_EYE_INDEX_POST_VERTEX(i);
        i.uv = SSRStereoTransformScreenSpaceTex(i.uv);
        // Raw dump of _TransparentNormalsRT so it can be compared 1:1 with Frame Debugger.
        half4 normals = SAMPLE_TEXTURE2D_X_LOD(_TransparentNormalsRT, sampler_PointClamp, i.uv, 0);
        return normals;
    }
