...
Version 32.3.4
- Clarified Custom Smoothness/Metallic Pass options in the inspector
- [Fix] Custom Smoothness/Metallic Pass not working in forward (32.3 regression)

Version 32.3.3
- [Fix] Reflection intensity on very first contact pixels

Version 32.3.2
- [Fix] Reflections on geometry with baked vertex positions in forward using Reflections scripts

Version 32.3.1
- [Fix] Auto-migration for the renamed Transparent Support component: scenes upgraded from 32.2 or earlier no longer fail to compile after import

Version 32.3
- Transparency Support has been moved to the Shiny SSR Render Feature inspector. The equivalent options on the Post Processing Volume are now deprecated and will be removed in a future release; reopen the Render Feature to reconfigure transparency
- New "Depth Prepass" toggle on the Render Feature with its own Layer Mask: renders the depth of selected transparent objects so SSR rays can hit them during ray-marching. Disabled by default. Works the same in forward and deferred. NOTE: projects upgrading from 32.2 or earlier that relied on Transparency Support must re-enable Depth Prepass in the Render Feature to keep the previous behaviour
- New "Screen Space Normals" group on the Render Feature with master toggle plus Opaques and Transparents sub-options. The Transparents toggle (with its own Layer Mask) replaces the previous deferred-only Normal Prepass: SSR now reads smooth normals at transparent pixels in both forward and deferred (Crest Water and similar shaders supported in both paths)
- New "Smoothness" slider that controls reflection sharpness on transparent pixels driven by the screen space normals
- New debug view "Transparent Normals" to inspect the transparent normals buffer

Version 32.2
- Improved raymarch algorithm accuracy
- VR Single Pass Instanced / Multiview: improved support across SSR shaders, Exclude Reflections pass and temporal accumulation buffer

Version 32.1.1
- [Fix] Fixed shader compile error on Unity 6000.0.x (GBufferCommon.hlsl include only available from Unity 6000.1+)

Version 32.1
- Raymarch accuracy improvements
